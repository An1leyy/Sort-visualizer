...
class Bar:
    ...
    def draw_value(self, screen):
        """
        Выводит числовое значние
        соответствующее колонке под ней.
        """
        ...
        pg.draw.rect(
            bg_surface, (*BLACK, TEXT_BG_ALPHA),
            (0, 0, bg_width, bg_height), border_radius=border_radius
        )
    ...
    def format_value(self, value):
        """
        Возвращает число в научной форме
    ...
    def move_to_index(self, current_index, target_index):
        """Перемещение колонки на указанный индекс."""

        self.target_x = self.x + (
            (target_index - current_index) * (self.width + SPACE_BETWEEN_BARS)
        )
    ...
    def is_animating(self):
        """Проверка на то, анимируется ли колонка в данный момент."""

        return self.is_moving or self.is_highlighted

def get_bar_height(value, min_value,
    max_value, min_height=BAR_MIN_HEIGHT,
    max_height=BAR_MAX_HEIGHT):
    """
    Вычисляет высоту колонки относительно
